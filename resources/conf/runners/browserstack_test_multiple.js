		...
		default:
			...
			for (let cap in browserCaps) {
				if(cap == "browsers")
				{
					...
					newKeys=browserCaps[cap];	
					
					for(let subcap in browserCaps[cap])
					{
						
						var newKey=key+"-"+browserCaps[cap][subcap];
						...
						browsers[newKey]["desiredCapabilities"]["browserName"]=browserCaps[cap][subcap];
						newKeys[subcap]=newKey;
					...
					if(multiBrowserFlag)
					{
						for(let newKey in newKeys)
						{
							
							var tweakedKey=newKeys[newKey];
							
							browsers[tweakedKey]["desiredCapabilities"][cap]=	browserCaps[cap];
			...
			if(multiBrowserFlag)
					{
						...
						{
							var tweakedKey=newKeys[newKey];
							browsers[tweakedKey]["desiredCapabilities"][
...
console.log("Final Capabilities => "+JSON.stringify(browsers));
